...
LOG
5/7/2022    - plot corrected for a delimited size per sample number
            - file management automatic with os.path (pathlib not compatible CV2)
...
def FirstFrame(folder, vidName): 
    ...
    while success:
        ...
        midDir = "BATTSI-{}".format(vidName.split('.')[0])
        imageDir = os.path.join(folder, midDir, fFrameName)
        #imageDir = folder +"\\BATTSI-{}\\{}".format(vidName.split('.')[0],fFrameName)
        print(imageDir)
...
while isFile != True:
    videoName = str(input('-> Video file (MyVideo.mp4): '))
...
middleDir = "BATTSI-{}".format(videoName.split('.')[0])
...
while invalidCalibration:
    calibrate = input('use default (d) or manual (m) calibration?:')
...
barras = df.plot(x='seconds', y=colnames[:-1], figsize=(10,4*sampleSize), subplots = True)
...
logName = "{}-Log-i{}-k{}.txt".format(videoName.split('.')[0],dilationIter, blurKernel)
